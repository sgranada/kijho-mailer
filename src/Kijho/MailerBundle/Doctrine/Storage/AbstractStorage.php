...
namespace Kijho\MailerBundle\Doctrine\Storage;

use Kijho\MailerBundle\Exception\InvalidArgumentException;
    ...
    protected function assertModelSupported($model) {
        if (false == $this->support($model)) {
            throw new InvalidArgumentException(sprintf(
                    'Invalid model given. Should be instance of %s but it is %s', $this->modelClass, is_object($model) ? get_class($model) : gettype($model)
            ));
        }
    }
